...
def demo_checkpoint_resume():
    ...
    try:
        result1 = supervisor_app.invoke({"messages": [HumanMessage(content="能帮我看一下47 Darling Street Glebe NSW Sydney这套房子的基本信息以及未来的走势吗？用互联网上的数据做一些合理的预测，比如3年后卖了能价值多少？我没有这套房子的基本信息，请查阅互联网然后告诉我，并且考虑所有可能会影响房价走势的因素，并预测3年左右的走势")], "sender": "user"}, config=config1)
        print(f"First execution completed")
        print("\n🔄 USER INTERRUPTION: User wants to clarify")
        
        # 第二次执行 - 使用相同的 thread_id 恢复状态
        print("\n📝 SECOND EXECUTION (Resume from checkpoint):")
        result2 = supervisor_app.invoke({
            "messages": [HumanMessage(content="等一下，我其实更关注是glebe 这个suburb的house其实，以及他们未来5年内的价值走向")], 
